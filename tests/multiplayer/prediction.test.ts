...
test("unacknowledged turning is replayed and acknowledged turning is not repeated", () => {
  ...
  prediction.advance(view, { ...freshControls(), yawDelta: 0.2 }, STEP, 10);
  expect(view.players[0]?.yaw).toBeCloseTo(yaw + 0.26);
  const snapshot = state();
  prediction.reconcile(snapshot, 9);
  expect(snapshot.players[0]?.yaw).toBeCloseTo(yaw + 0.26);
