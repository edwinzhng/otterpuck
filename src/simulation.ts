...
const advancePuck = (state: Simulation, dt: number): void => {
  const puck = state.puck;
  ...
  if (puck.controlOwner === undefined && puck.shotOwner === undefined) {
    ...
    if (puck.flightOrientation) {
      const landing =
        puck.velocity.y < 0
          ...
          : 0;
      puck.orientation
        .copy(puck.flightOrientation)
        .slerp(new Quaternion(), landing);
    }
    ...
    if (angularSpeed > 0.001)
      puck.orientation
        .premultiply(
          new Quaternion().setFromAxisAngle(
            puck.angularVelocity.clone().divideScalar(angularSpeed),
            angularSpeed * dt,
          ),
        )
        .normalize();
