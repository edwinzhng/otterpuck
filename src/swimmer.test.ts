...
import { bladePoint, HOOK_ROOT, STICK_GRIP } from "./stick";
import { loadFirstPersonModel } from "./test-support/assets";
import { type Controls, freshControls, STEP, STICK_EDGE } from "./types";
import { createSwimmerView, poseSwimmer } from "./world";

const asset: { template: Object3D | undefined } = { template: undefined };
beforeAll(async (): Promise<void> => {
  asset.template = (await loadFirstPersonModel()).scene;
});
const setup = (): {
