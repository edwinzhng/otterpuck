...
export const loadArena = async (id: ArenaId): Promise<ArenaView> => {
  ...
  gltf.scene.traverse((object): void => {
    ...
    const convert = (source: Material): Material => {
      ...
      const result = source.name.startsWith("Distant ocean")
        ? createOceanSurface()
        : source.name === "Pool floor" || source.name === "Pool wall"
          ? createPoolSurface(city, source.name === "Pool wall")
          : source instanceof MeshStandardMaterial && source.metalness < 0.1
  ...
  root.add(
    new HemisphereLight(
      ...
      city ? 0x425672 : 0xb7c9d4,
      city ? 0.42 : 0.26,
    ),
  );
  const sun = new DirectionalLight(
    city ? 0xd2d9ff : 0xffffff,
    city ? 2.3 : Math.PI * 0.9,
  );
  ...
  return {
    ...
    time,
    shaders,
    fog: new Color(city ? 0x327d9c : 0x43b5c7),
    air: new Color(city ? 0x465578 : 0xb7e5ea),
    rockTexture,
  };
};
